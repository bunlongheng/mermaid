...
} from '/var/task/Sites/mermaid/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/Sites/mermaid/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/Sites/mermaid')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/Sites/mermaid',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/Sites/mermaid/.netlify/dist/run/handlers/server.js')
